...
// TestTestNet5GenesisBlock tests the genesis block of the test network (version
// 3) for validity by checking the encoded bytes and hashes.
func TestTestNet5GenesisBlock(t *testing.T) {
	// Encode the genesis block to raw bytes.
	var buf bytes.Buffer
	err := TestNet5Params.GenesisBlock.Serialize(&buf)
	if err != nil {
		t.Fatalf("TestTestNet5GenesisBlock: %v", err)
	}
	...
	// Check hash of the block against expected hash.
	hash := TestNet5Params.GenesisBlock.BlockHash()
	if !TestNet5Params.GenesisHash.IsEqual(&hash) {
		t.Fatalf("TestTestNet5GenesisBlock: Genesis block hash does "+
			"not appear valid - got %v, want %v", spew.Sdump(hash),
			spew.Sdump(TestNet5Params.GenesisHash))
	}
}
